...
//package edu.berkeley.nlp.assignments.assign1.student {
  ...
  class NgramCounter
  {
    ...
    //
    //
    val unigramSet = new CompactHashSet[Int]()
    val bigramSet = new CompactHashSet[Long]()
    val trigramSet = new CompactHashSet[Long]()
    ...
    def getCount(ngram: Array[Int]) = 0
    ...
    def incCount(ngram: Array[Int]) = {
      val key = ngram.mkString(" ")
      if (ngram.length == 1) {
        val key = ngram(0)
        unigramSet.add(key)
      } else if (ngram.length == 2) {
        val key = ngram(0).toLong | (ngram(1).toLong << 20)
        bigramSet.add(key)
      } else if (ngram.length == 3) {
        val key = ngram(0).toLong | (ngram(1).toLong << 20) | (ngram(2).toLong << 40)
        trigramSet.add(key)
      } else {
  ...
  class KNTrigramLanguageModel(sentenceCollection: java.lang.Iterable[java.util.List[String]]) extends NgramLanguageModel
  {
    ...
    def getNgramProbability(ngram: Array[Int], from: Int, to: Int): Double = {
      if (to - from == 1) {
        ...
        val trigramCount = Math.max(this.getCount(ngram.slice(from, to)), smoothingFactor)
        val prefixCount = this.getCount(ngram.slice(from, to - 1)) + smoothingFactor
